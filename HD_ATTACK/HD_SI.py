...
class HD_SI:
    ...
    def train_T(self, train_model, input, true_label, writer, loss_mode, T_Net_name, global_step):
        ...
        while step < steps:
            ...
            loss_f = (loss_f1 + loss_f2 + loss_f3 + loss_f4 + loss_f5)/5 

            loss_T1 = self.loss_fn(model(self.T_net(input_var)), label_var)
            loss_T2 = self.loss_fn(model(self.T_net(1/2 * input_var)), label_var)
            loss_T3 = self.loss_fn(model(self.T_net(1/4 * input_var)), label_var)
            loss_T4 = self.loss_fn(model(self.T_net(1/8 * input_var)), label_var)
            loss_T5 = self.loss_fn(model(self.T_net(1/16 * input_var)), label_var)
            loss_T = (loss_T1 + loss_T2 + loss_T3 + loss_T4 + loss_T5)/5 
            # **********************************************************************
